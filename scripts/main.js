...
function formatNumberPDF(valuePDF) {
  ...
  // Then round *croreValue* to nearest integer
  let roundedValue = Math.round(croreValue);

  // Format number with Indian commas but no decimals
...
async function getAllInputValues() {
  ...
  for (let i = 0; i < parseFloat(safeParseFloat(grossIntendedPeriod)); i++) {
    ...
    // totalDutyOnRMGE =
    // totalDutyOnRMGE * (safeParseFloat(ExpectedAnnualGrowth) / 100 + 1);
  }
  ...
  // console.log("17) total Duty On RMGE", lastNPVRawDuty);
  ...
  let NetBeniftForMOOWR =
    totalDuty +
    (RowTwoThirdCell == "Nil" ? 0 : RowTwoThirdCell) +
    totalSavings +
    lastNPV +
    lastNPVigst +
    lastNPVtotalBenifit +
...
async function updatePDFAndDownload(
  ...
  // const boldFont = await pdfDoc.embedFont(PDFLib.StandardFonts['Helvetica-Bold']); // Bold
